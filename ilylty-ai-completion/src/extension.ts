...
const triggerState = {
    active: false,
    position: null as vscode.Position | null,
    documentVersion: 0
};
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('ilylty.AI.completion.trigger', async () => {
    ...
    const provider: vscode.InlineCompletionItemProvider = {
        ...
        async generateCompletions(line: string, pos: vscode.Position, filePath: string) {
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json() as CompletionResponse; // 使用定义的接口
